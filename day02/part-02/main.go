...
func main() {
	...
	for _, report := range reports {
		fmt.Println(report)
		if checkSafeReport(report) || checkSafeReportWithToleration(report) {
			safeReports += 1
		}
	}
...
func checkSafeReportWithToleration(report []int) bool {
	for index := range report {
		if checkSafeReport(removeIndex(report, index)) {
			return true
		}
	}
	return false
